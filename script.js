document.addEventListener("DOMContentLoaded", () => {
    ...
    .then(([comandi, direzioni, con, socav, prefissi, lingue, moduliCMR, linkUtili]) => {
        ...
        // Popolano un pannello ora ospitato in messaggistica-bis.js: chiamate
        // via window perché quel file, caricato dopo, le espone lì.
        if (window.popolaSelectPrefissoMsg) window.popolaSelectPrefissoMsg(prefissiData);
        if (window.popolaSelectLinguaMsg) window.popolaSelectLinguaMsg(lingueData);
    ...
    function attivaComando(nomeComando) {
        ...
        // Il messaggio precompilato dipende dal Comando attivo: messaggistica-bis.js
        // lo rigenera da sé, ascoltando l'evento "fireops:comando-attivo-cambiato"
        // dispatchato due righe sotto (stessa via già usata da convertitore.js).
...
const CHIAVE_STORAGE_PANNELLI = "fireops_pagine_pannelli";
    ...
    function etichettaScheda(pagina) {
        return pagina.lavori ? `🚧 Pagina in costruzione! 🚧 ${pagina.label}` : pagina.label;
    }
    ...
    function eseguiEffettiPagina(idPagina) {
        ...
        if (idPagina === "messaggistica") {
            ...
            if (window.validaCampiMessaggistica) window.validaCampiMessaggistica();
        }
...
salvaPaginePannelli();
    ...
    // Wrapper minimo su FireOps.creaCombo: usato da Moduli CMR e da Sostanze
    // Pericolose qui sotto. Una copia identica vive anche in
    // messaggistica-bis.js (per Prefisso/Lingua) — duplicarla è più semplice
    // e sicuro che condividerla tra due file separati per una funzione di
    // una riga.
    function creaComboRicercabile(opzioni) { return FireOps.creaCombo(opzioni); }

    // Riferimenti DOM di Moduli CMR: qui, non spostati con la Messaggistica
    // in messaggistica-bis.js — questa pagina non c'entra con quella.
...
window.renderIcscSelezionata = renderIcscSelezionata;
    ...
    function updateClockAndShift() {
        ...
        const offsetOre = FireOps.offsetOreRoma(now);
