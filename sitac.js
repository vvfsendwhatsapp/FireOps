...
function avvia(app){
  ...
  /* I sei gruppi della tavola si raccolgono in quattro categorie
     operative: è così che si ragiona davanti a una carta — dove sto,
     cosa fa il fuoco, chi ho in campo, cosa sto facendo. */
  const CATEGORIE = [
    {k:'cZona',        gruppi:['viabilita','zona']},
    {k:'cIncendio',    gruppi:['incendio']},
    {k:'cDispositivo', gruppi:['aerei','terra']},
    {k:'cAzioni',      gruppi:['azioni']}
  ];
  ...
  function creaPulsanti(){
    ...
    /* Primo livello: la geometria (Punti, Linee, Aree) — è la prima cosa
       che si decide, perché determina come si disegna. Dentro, le quattro
       categorie operative della tavola. */
    ...
    fisa(t('gPunti'), corpo => {
      ...
      CATEGORIE.forEach(cat => {
        const voci = Object.entries(SIM).filter(([, d]) => cat.gruppi.includes(d.g));
        if (!voci.length) return;
        corpo.insertAdjacentHTML('beforeend',
          `<span class="sitac-sottogruppo">${esc(t(cat.k))}</span>`);
    ...
    fisa(t('gLinee'), corpo => {
      let n = 0;
      CATEGORIE.forEach(cat => {
        const voci = Object.entries(LIN).filter(([, d]) => cat.gruppi.includes(d.g));
        if (!voci.length) return;
        corpo.insertAdjacentHTML('beforeend',
          `<span class="sitac-sottogruppo">${esc(t(cat.k))}</span>`);
    ...
    fisa(t('gAree'), corpo => {
      corpo.insertAdjacentHTML('beforeend',
        `<span class="sitac-sottogruppo">${esc(t('cPerimetri'))}</span>`);
      const el = document.createElement('div');
      el.className = 'sitac-strumenti';
  ...
  const nomeFile = est =>
    `sitac_${new Date().toISOString().slice(0,16).replace(/[:T-]/g,'')}.${est}`;
  ...
  $('bGeojson').onclick = () => {
    ...
    const fc = {type:'FeatureCollection', features:feat,
      properties:{applicazione:'FireOps VVF — SITAC', simbologia:'SITAC CNVVF',
    ...
    L.geoJSON(fc, {
      pointToLayer: (feat, latlng) => {
        ...
        return L.marker(latlng, {draggable:true,
          icon: iconaSimbolo(p.tipo, {stato:p.stato || 'previsto',
